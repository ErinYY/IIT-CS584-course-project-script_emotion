...
use_full = False
model_path = 'multi_regression_model_qa_context.pkl'
max_epoch = 3
batch_size = 10
path = os.getcwd()
device = torch.device('cuda:0') if torch.cuda.is_available() else torch.device('cpu')

train = pd.read_csv('train_dataset_with_contentid.tsv', sep=',', error_bad_lines=False, warn_bad_lines=False)
print(train.shape)
print(train.head())

test = pd.read_csv('test_dataset_with_contentid.tsv', sep=',')
print(test.shape)
print(test.head())
...
train_data = full_data_shuff if use_full else full_data_shuff.iloc[:split_pos,:]
val_data = full_data_shuff.iloc[split_pos:,:]
class MyDataSet(Dataset):
    def __init__(self,texts,characters,content_ids,uniq_contents,labels=None):
        self.texts = list(texts)
        self.characters = list(characters)
        self.content_ids = content_ids
        self.uniq_contents = uniq_contents
    ...
    def __getitem__(self, index):
        ...
        texts_with_context = []
        if content_id>1:
            texts_with_context.extend(self.uniq_contents[content_id - 2:content_id+1])
        elif content_id>0:
            texts_with_context.extend(self.uniq_contents[content_id - 1:content_id + 1])
        else:
            texts_with_context.append(self.uniq_contents[content_id])
        texts_with_context = ''.join(texts_with_context)
        if self.labels is not None:
            return [texts_with_context,self.characters[index],self.labels[index]]
        else:
            return [texts_with_context,self.characters[index]]

    def __len__(self):
        return len(self.texts)

with open('train_uniq_content.tsv','r',encoding='utf-8') as rf:
    uniq_contents = rf.readlines()
with open('test_uniq_content.tsv', 'r', encoding='utf-8') as rf:
    test_uniq_contents = rf.readlines()
train_dataset = MyDataSet(train_data['text'].values,characters=train_data['character'].values,
                          content_ids=train_data['content_id'].values,uniq_contents=uniq_contents,
...
val_dataset = MyDataSet(val_data['text'].values,characters=val_data['character'].values,
                        content_ids=val_data['content_id'].values,uniq_contents=uniq_contents,
...
test_dataset = MyDataSet(test['text'].values,characters=test['character'].values,
                         content_ids=test['content_id'].values,uniq_contents=test_uniq_contents,)
...
if is_train:
    ...
    sub.head()
    sub.to_csv('baseline_qa_context7.tsv', sep='\t', index=False)